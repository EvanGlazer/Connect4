...
{
	private Map<int[], Integer> mem = new HashMap<int[], Integer>();
	...
	int DoSearch( Position pos, int nPiece, int nDepth, Board pBoard, int nAlpha, int nBeta )
	{
		...
		// Check if result already in mem
		if ( mem.containsKey( nMoves ) ) {
			return mem.get( nMoves ).intValue();
		}
